...
class AverageSalary:
    def __init__(self, key, average, participant_count):
        ...
        self.particpant_count = participant_count
...
def main():
    ...
    industry_more_than_10 = []

    
    for group in average_salaries_by_industry:
        if group.particpant_count >= 10:
            industry_more_than_10.append(group)
            

    sorted_industry_more_than_10 = sorted(industry_more_than_10, key = lambda x: x.average, reverse=True)

    top_5_industries = []
    for entry in sorted_industry_more_than_10[:5]:
...
# avg salory by years of experience
    years_of_experince_groups = group_by_attribute(participants, "experience")
    average_salaries_by_experince = get_average_salary(years_of_experince_groups)
    
    
    for group in average_salaries_by_experince:
        print("Answer #5",group.key, group.average)
